...
function pindahKeSection(url, idSection) {
    // Gabungkan URL dengan hash fragment
    const fullUrl = `${url}#${idSection}`;
    
    // Pindah ke URL dan bagian yang dituju
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const boxes = document.querySelectorAll('.package-content .box-package');
...
sr.reveal ('.first-box',{delay:100});
sr.reveal ('.conten-cover',{delay:100});
sr.reveal ('.article-detail',{delay:100});
sr.reveal ('.first-box',{delay:100});
sr.reveal ('.conten-cover',{delay:100});
sr.reveal ('.content-box',{delay:100});
sr.reveal ('.article-footer',{delay:100});
